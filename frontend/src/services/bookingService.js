...
export const bookingService = {
  ...
  normalizeDiscounts: (backendDiscounts = []) => {
    ...
    return backendDiscounts.map((discount) => {
      ...
      minimumMemberTier: discount.minimumMemberTier ?? 'V_STAR',
      customerSegment: discount.customerSegment ?? 'ALL',
      applicableChannels: discount.applicableChannels ?? 'CINEMA',
      applicableWeekdays: discount.applicableWeekdays ?? '',
      perUserLimit: Number(discount.perUserLimit) || 1,
      budgetLimit: Number(discount.budgetLimit) || 0,
      budgetUsed: Number(discount.budgetUsed) || 0,
