...
import {
  ...
  FormControlLabel, IconButton, LinearProgress, Pagination, Paper, Stack, Switch, Tab, Tabs,
...
const ACCENT_SOFT = 'rgba(245,181,27,.13)';
const SURFACE = '#151d2c';
...
const Photo = ({ person, height, radius = 0 }) => <Box sx={{ height, overflow: 'hidden', borderRadius: radius, bgcolor: '#111827' }}>
  {person?.avatarUrl ? <Box component="img" src={person.avatarUrl} alt={person.fullName || 'Hồ sơ hẹn hò'} sx={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', userSelect: 'none', pointerEvents: 'none' }} />
    : <Stack alignItems="center" justifyContent="center" sx={{ height: '100%', background: 'linear-gradient(145deg, #1d293a, #111827)' }}><Avatar sx={{ width: 130, height: 130, fontSize: 54, color: '#111827', background: 'linear-gradient(145deg, #ffd76a, #f5a800)' }}>{firstLetter(person?.fullName)}</Avatar></Stack>}
</Box>;
...
const FloatingAction = ({ kind, disabled, onClick }) => {
  ...
  return <Tooltip title={like ? 'Thích hoặc vuốt phải' : 'Bỏ qua hoặc vuốt trái'}><span><IconButton
    ...
    sx={{ width: like ? 78 : 70, height: like ? 78 : 70, background: like ? 'linear-gradient(145deg, #ffd052, #f5a800)' : 'rgba(15,23,42,.94)', color: like ? '#111827' : '#f8fafc',
      border: like ? 0 : '1px solid rgba(255,255,255,.15)', boxShadow: like ? '0 12px 30px rgba(245,181,27,.3)' : '0 10px 26px rgba(0,0,0,.28)',
      '&:hover': { background: like ? `linear-gradient(145deg, ${ACCENT}, ${ACCENT_DARK})` : '#263247' } }}>
...
const DiscoveryCard = ({ person, nextPerson, busy, onDecision }) => {
  const [drag, setDrag] = useState({ active: false, x: 0, leaving: false });
  const gestureRef = useRef({ active: false, startX: 0, x: 0, pointerId: null });
  ...
  const updateVisual = () => {
    ...
    setDrag({ active: true, x: gestureRef.current.x, leaving: false });
  };
  const moveDrag = (event) => {
    const gesture = gestureRef.current;
    if (!gesture.active || gesture.pointerId !== event.pointerId) return;
    gesture.x = event.clientX - gesture.startX;
    if (!frameRef.current) frameRef.current = requestAnimationFrame(updateVisual);
  };
  const finishDrag = (event) => {
    ...
    const finalOffset = gesture.x;
    if (Math.abs(finalOffset) >= 90 && !busy) {
      const decision = finalOffset > 0 ? 'LIKE' : 'PASS';
      const exitX = (finalOffset > 0 ? 1 : -1) * Math.max(window.innerWidth, 720);
      setDrag({ active: false, x: exitX, leaving: true });
      window.setTimeout(() => onDecision(decision), 180);
    } else {
      setDrag({ active: false, x: 0, leaving: false });
    }
  };
  ...
  return <Box sx={{ width: '100%', maxWidth: 600, height: { xs: 625, sm: 700 }, position: 'relative' }}>
    ...
    <Card onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); gestureRef.current = { active: true, startX: event.clientX, x: 0, pointerId: event.pointerId }; setDrag({ active: true, x: 0, leaving: false }); }}
      ...
      sx={{ position: 'absolute', inset: 0, overflow: 'visible', borderRadius: 5, bgcolor: SURFACE, color: '#f8fafc', cursor: drag.active ? 'grabbing' : 'grab', touchAction: 'pan-y', userSelect: 'none',
...
const Shortcut = ({ icon, label, onClick, active = false }) => <Button onClick={onClick} sx={{ minWidth: 0, color: active ? '#ffd052' : 'rgba(226,232,240,.72)', textTransform: 'none', flexDirection: 'column', gap: .8, fontWeight: 800 }}>
  <Box sx={{ width: 58, height: 58, display: 'grid', placeItems: 'center', borderRadius: '50%', background: active ? 'linear-gradient(145deg, #ffd052, #f5a800)' : 'linear-gradient(145deg, #202b3d, #151d2c)', color: active ? '#111827' : '#ffd052', border: active ? '1px solid transparent' : '1px solid rgba(255,255,255,.1)', boxShadow: active ? '0 8px 20px rgba(245,181,27,.25)' : 'none', transition: 'all .2s ease' }}>{icon}</Box><Typography variant="caption" fontWeight={850}>{label}</Typography>
</Button>;

const MatchTile = ({ match, onOpen }) => <Button onClick={onOpen} sx={{ minWidth: 0, p: 0, textTransform: 'none', color: '#f8fafc', display: 'block', textAlign: 'left' }}>
  <Box sx={{ width: '100%', aspectRatio: '4 / 5', position: 'relative' }}><Photo person={match.person} height="100%" radius={3} /><Box sx={{ position: 'absolute', inset: '48% 0 0', borderRadius: 3, background: 'linear-gradient(transparent,rgba(0,0,0,.9))' }} /><Typography fontWeight={900} color="white" noWrap sx={{ position: 'absolute', left: 10, right: 8, bottom: 8 }}>{match.person?.fullName}</Typography></Box>
...
const MatchesCarousel = ({ matches, onOpen }) => {
  ...
  const scroll = (direction) => {
    ...
    if (!rail) return;
    rail.scrollBy({ left: direction * Math.max(rail.clientWidth * 0.78, 320), behavior: 'smooth' });
  };
  ...
  return <Box sx={{ position: 'relative' }}>
    <Stack direction="row" spacing={1} sx={{ position: 'absolute', right: 0, top: -48, zIndex: 2 }}>
      <IconButton aria-label="Xem kết nối trước" onClick={() => scroll(-1)} sx={{ width: 34, height: 34, color: '#f8fafc', bgcolor: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', '&:hover': { bgcolor: ACCENT_SOFT, color: '#ffd052' } }}>
        <ArrowBackIosNewRoundedIcon sx={{ fontSize: 16 }} />
      </IconButton>
      <IconButton aria-label="Xem kết nối tiếp theo" onClick={() => scroll(1)} sx={{ width: 34, height: 34, color: '#111827', bgcolor: ACCENT, '&:hover': { bgcolor: '#ffd052' } }}>
        <ArrowForwardIosRoundedIcon sx={{ fontSize: 16 }} />
      </IconButton>
    </Stack>
    <Box
      ref={railRef}
      aria-label="Danh sách kết nối mới"
      ...
      sx={{
        display: 'grid',
        gridAutoFlow: 'column',
        gridAutoColumns: { xs: 'minmax(138px, 43vw)', sm: '158px', md: '170px' },
        gap: 1.6,
        overflowX: 'auto',
    ...
    >
      {matches.map((match) => <MatchTile key={match.matchId} match={match} onOpen={() => onOpen(match)} />)}
    </Box>
  </Box>;
};
...
export default function MovieMatchingPanel() {
  ...
  return <Box sx={{ color: '#f8fafc',
    ...
    {loading ? <Box textAlign="center" py={10}><CircularProgress sx={{ color: ACCENT }} /></Box> : <>
      ...
      {tab === 1 && <Paper sx={{ p: { xs: 2, sm: 3.5 }, borderRadius: 5, bgcolor: SURFACE, border: '1px solid rgba(255,255,255,.08)', boxShadow: '0 20px 60px rgba(0,0,0,.24)' }}>
